...
const createPaymentIntent = async (amount, currency = 'usd') => {
  ...
  /**
   * Renders the complete project page.
   * @param {object} project - The project data object.
   */
  render(project) {
    // 1. Update meta information (page title, hero section).
    this._updateMeta(project);

    // 2. Build the main content HTML.
    ...
    container.innerHTML = `
            <div class="project-content">
                ...
                <div class="project-details">
                    ...
                    <div class="tech-stack">${project.technologies.map(tech => `<span class="tech-item">${tech}</span>`).join('')}</div>
                    <div class="features">
                        <h3>Key Features</h3>
                        <ul class="features-list">${project.features.map(feature => `<li><i class="fas fa-check-circle"></i> ${feature}</li>`).join('')}</ul>
                    </div>
                </div>
            </div>
            ${this._renderTimeline(project.timeline)}
            ${this._renderChallenges(project.challenges)}
            ${this._renderArchitecture(project.architecture)}
            ${this._renderCodeSnippets(project.codeSnippets)}
            ${this._renderGallery(project.gallery)}
            ${this._renderMetrics(project.metrics)}
            ${this._renderLessons(project.lessons)}
            ${this._renderRelatedProjects(project.relatedProjects)}
        `;

    // 3. Initialize animations for newly added elements.
    this._initAnimations();
  },

  /**
   * Renders a "Not Found" message if the project ID is invalid.
   */
  renderNotFound() {
    document.title = "Project Not Found";
    document.getElementById('hero-title').textContent = "Oops! Project Not Found";
    document.getElementById('hero-subtitle').textContent = "404 Error";
    const container = document.getElementById('project-details');
    if (container) {
      container.innerHTML = `
                <div class="not-found">
                    <p>The project you're looking for doesn't exist or has been moved.</p>
                    <a href="index.html#portfolio" class="back-btn"><i class="fas fa-arrow-left"></i> Back to Portfolio</a>
                </div>
            `;
    }
  },
  ...
  /**
   * Updates the page title and hero section content.
   * @param {object} project - The project data object.
   */
  _updateMeta(project) {
    ...
    heroButtons.innerHTML = `
            <a href="${project.github}" class="hero-btn primary" target="_blank" rel="noopener noreferrer"><i class="fab fa-github"></i> View Source Code</a>
            <a href="${project.demo}" class="hero-btn secondary" target="_blank" rel="noopener noreferrer"><i class="fas fa-external-link-alt"></i> Live Demo</a>
        `;
  },

  /**
   * Sets up IntersectionObserver to trigger animations on scroll.
   */
  _initAnimations() {
    const animatedElements = document.querySelectorAll('.timeline-item, .challenge-solution-pair, .metric-card, .lesson-item, .related-card');
  ...
  /**
   * Generates HTML for the project timeline section.
  ...
  _renderArchitecture(architecture) {
  ...
  _renderCodeSnippets(codeSnippets) {
            ...
            <div class="code-showcase-section">
                ...
                ${codeSnippets.map(snippet => `
                    <div class="code-showcase">
                        ...
                        <pre><code>${this._escapeHtml(snippet.code)}</code></pre>
                    </div>`).join('')}
            </div>`;
  },
    ...
    return `
            <div class="gallery-section">
                ...
                <div class="gallery-grid">
                    ${gallery.map(item => `
                        <a href="${item.image}" target="_blank" class="gallery-item">
    ...
    return `
            <div class="lessons-section">
                ...
                <ul class="lessons-list">
                    ${lessons.map(lesson => `<li><i class="fas fa-star"></i>${lesson}</li>`).join('')}
                </ul>
            </div>`;
  },

  _renderRelatedProjects(relatedProjects) {
            ...
            <div class="related-projects">
                ...
                <div class="related-grid">
                    ${relatedProjects.map(project => `
                        <a href="project.html?id=${project.id}" class="related-card">
                            <img src="${project.image}" alt="${project.title}" class="related-image" loading="lazy">
  ...
  /**
   * Escapes HTML special characters to prevent XSS and display code correctly.
